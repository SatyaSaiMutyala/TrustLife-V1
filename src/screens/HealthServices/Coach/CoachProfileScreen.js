...
const CoachProfileScreen = () => {
  ...
  const handleBook = () => {
    ...
    const appointment = {
      id: 'S' + Date.now(),
      coachName: coach?.name || 'Dr. Priya Sharma',
      specialty: coach?.specialty || 'Nutrition Coach',
      date: `2026-02-${days[selectedDay].date}`,
      time: slot?.time || '10:00 AM',
      status: 'Confirmed',
      sessionType: 'Video Consultation',
      amount: coach?.fee || 800,
      paymentMethod: 'UPI',
      transactionId: 'TXN' + Date.now(),
      address: 'Online',
      rating: coach?.rating || 4.8,
    };
    navigation.navigate('CoachSessionDetail', {appointment});
  };
        ...
        <View style={styles.profileSection}>
          ...
          <View style={styles.specialtyPill}>
            <AppText variant="small" color={Colors.primary} style={{fontWeight: '600'}}>
              {coach?.specialty || 'Nutrition Coach'}
            </AppText>
          </View>

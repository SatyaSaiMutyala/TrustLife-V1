...
const CounsellingProfileScreen = () => {
  ...
  const handleBook = () => {
    ...
    const appointment = {
      id: 'CS' + Date.now(),
      counsellorName: counsellor?.name || 'Dr. Meera Joshi',
      specialty: counsellor?.specialty || 'Stress Counselling',
      date: `2026-02-${days[selectedDay].date}`,
      time: slot?.time || '10:00 AM',
      status: 'Confirmed',
      sessionType: 'Video Consultation',
      amount: counsellor?.fee || 1200,
      paymentMethod: 'UPI',
      transactionId: 'TXN' + Date.now(),
      address: 'Online',
      rating: counsellor?.rating || 4.9,
    };
    navigation.navigate('CounsellingSessionDetail', {appointment});
  };
      ...
      <View style={styles.header}>
        ...
        <View style={styles.profileSection}>
          <View style={styles.avatarLarge}>
            {counsellor?.image ? (
              <Image source={counsellor.image} style={styles.avatarImage} resizeMode="cover" />
            ) : (
          ...
          {mockTimeSlots.map(slot => {
            ...
            return (
              <TouchableOpacity
                ...
                <AppText
                  variant="caption"
                  color={isBooked ? '#E53935' : isSelected ? Colors.primary : Colors.tealText}

...
const PhysioProfileScreen = () => {
  ...
  const handleBook = () => {
    ...
    const appointment = {
      id: 'S' + Date.now(),
      physioName: physio?.name || 'Dr. Arjun Mehta',
      specialty: physio?.specialty || 'Sports Physiotherapy',
      date: `2026-03-${days[selectedDay].date}`,
      time: slot?.time || '10:00 AM',
      status: 'Confirmed',
      sessionType: 'Clinic Visit',
      amount: physio?.fee || 800,
      paymentMethod: 'UPI',
      transactionId: 'TXN' + Date.now(),
      address: 'Clinic Visit',
      rating: physio?.rating || 4.9,
    };
    navigation.navigate('PhysioSessionDetail', {appointment});
  };
      ...
      <View style={styles.header}>
        ...
        <View style={styles.profileSection}>
          <View style={styles.avatarLarge}>
            {physio?.image ? (
              <Image source={physio.image} style={styles.avatarLargeImage} resizeMode="cover" />
            ) : (
          ...
          <View style={styles.specialtyPill}>
            <AppText variant="small" color={Colors.primary} style={{fontWeight: '600'}}>
              {physio?.specialty || 'Sports Physiotherapy'}
            </AppText>
          </View>
...
export default PhysioProfileScreen;

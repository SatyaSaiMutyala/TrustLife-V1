...
const mockDoctors = [
  {
    ...
    fee: 499,
    image: require('../../../assets/img/doctor.jpg'),
  },
  {
    id: 'TD002',
    name: 'Dr. Rahul Mehra',
...
const filterChips = ['Experience', 'Consultation Fee', 'Rating', 'Language'];

const TelemedicineSpecialistListScreen = () => {
  ...
  const doctors = mockDoctors.map(d => ({...d, specialty: specialtyName || d.specialty}));

  const filteredDoctors = searchText
    ? doctors.filter(d => d.name.toLowerCase().includes(searchText.toLowerCase()))
    : doctors;
      ...
      <FlatList
        ...
        renderItem={({item}) => (
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            onPress={() => navigation.navigate('TelemedicineProfile', {doctor: item})}>
            {/* Online status dot */}
            ...
            <AppText variant="bodyBold" style={styles.doctorName} numberOfLines={1}>{item.name}</AppText>
            ...
            <View style={styles.videoBadge}>
              <AppText variant="small" color={Colors.tealText} style={{fontWeight: '600'}}>Video</AppText>
            </View>

            <AppText variant="bodyBold" color={Colors.primary} style={{marginTop: vs(6)}}>
              {'\u20B9'}499
            </AppText>
          </TouchableOpacity>
        )}
...
export default TelemedicineSpecialistListScreen;
